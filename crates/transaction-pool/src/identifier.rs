...
/// Trait for sender identifiers that uniquely identify transaction senders.
///
/// This trait allows for different identifier schemes - from simple address-based
/// identifiers to more complex schemes that incorporate nonce keys for parallel
/// transaction support.
pub trait SenderId:
    std::fmt::Debug
    + Copy
    ...
    + 'static
{
    ...
    fn into_transaction_id(self, nonce: u64) -> TransactionId<Self>
    where
        Self: Sized;
}
...
impl PoolSenderId {
    ...
    /// Returns a `Bound` for [`TransactionId`] starting with nonce `0`
    pub const fn start_bound(&self) -> std::ops::Bound<TransactionId<Self>> {
        std::ops::Bound::Included(TransactionId::new(*self, 0))
    }

    /// Converts the sender to a [`TransactionId`] with the given nonce.
    pub const fn into_transaction_id(self, nonce: u64) -> TransactionId<Self> {
...
/// Trait for managing sender identifier mappings.
///
/// Implementations of this trait control how addresses (and potentially other transaction
/// properties) are mapped to unique identifiers used internally by the transaction pool.
pub trait SenderIdentifiers: std::fmt::Debug + Send + Sync {
    /// The type of sender identifier used by this implementation.
    type Id: SenderId;

    /// Returns the address for the given identifier.
    fn address(&self, id: &Self::Id) -> Option<&Address>;

    /// Returns the identifier for the given transaction.
    ///
    /// This method allows implementations to use any transaction properties (address, nonce_key,
    /// etc.) to determine the identifier.
    fn sender_id_for_transaction<T: crate::traits::PoolTransaction>(&mut self, tx: &T) -> Self::Id;

    /// Returns existing identifier for the address or creates a new one.
    ///
    /// This is a convenience method for simple address-based lookups.
    fn sender_id_or_create(&mut self, addr: Address) -> Self::Id;
}
...
impl SenderIdentifiers for DefaultSenderIdentifiers {
    ...
    fn sender_id_for_transaction<T: crate::traits::PoolTransaction>(&mut self, tx: &T) -> Self::Id {
        // Default implementation: just use address, ignore nonce_key
        self.sender_id_or_create(tx.sender())
    }
...
impl<S: SenderId> TransactionId<S> {
    /// Create a new identifier pair
    pub const fn new(sender: S, nonce: u64) -> Self {
        Self { sender, nonce }
    }
    ...
    /// `on_chain_nonce`
    pub fn ancestor(transaction_nonce: u64, on_chain_nonce: u64, sender: S) -> Option<Self> {
...
mod tests {
    ...
    fn test_address_retrieval() {
        let mut identifiers = DefaultSenderIdentifiers::default();
    ...
    fn test_next_id_wrapping() {
        let mut identifiers = DefaultSenderIdentifiers { id: u64::MAX, ..Default::default() };
